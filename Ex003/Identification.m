...
fa = {f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14, f15, f16};
...
C = zp * Z' * inv(Z * Z');
% Example:
% Model Type 1. z=c1*(x)    + c2*(y)   + c3
F = @ (a, b) (C(1)*a + C(2)*b + C(3));
mt = mta(1);
sf = polyfitn([xpp, ypp],zpp, mt1); %'x y constant'
c = sf.Coefficients;
f1 = @ (a, b) (c(1)*a + c(2)*b + c(3));
f =  @ (a, b) (c(1)*a + c(2)*b + c(3));

% Model Type 2. z=c1*(x)    + c2*(y)   + c3*(x)*(y) + c(4)
%F = @ (a, b) (C(1)*a + C(2)*b + C(3)*a*b + C(4));
plot3(X, Y, z, 'xb');
hold on;
